...
#### Scenario: Brand Clients collection with only TotalCoin mode

- GIVEN the "Brand Clients" collection has a single default mode "TotalCoin" (other client modes excluded)
- WHEN the pipeline extracts this collection
- THEN it produces `tokens/source/brand-clients.tokens.json` with TotalCoin brand values
